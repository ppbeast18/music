...
export default function AudioRecognition() {
  ...
  // Function to send audio chunk to API
    ...
    try {
      ...
      const response = await fetch(
        'https://shazam-api-audio-recognition-for-songs-music-metadata.p.rapidapi.com/detect_audio_by_post',
        {
          method: 'POST',
          headers: {
            'x-rapidapi-key': '5152423465mshca6217211c386a4p1cc9a8jsn1d99759c4e2a',
      ...
      if (data.result) {
        ...
        setSuccess(true);
        stopRecording();
        // Clear recording duration and timer when result is found
      ...
      console.error('Error in song detection:', err);
    }
  };
    ...
    try {
      ...
      mediaRecorder.ondataavailable = async (event) => {
        if (event.data.size > 0) {
          ...
          if (!success) {
            // Send the latest 4 seconds of audio for recognition
            const combinedChunks = new Blob(audioChunksRef.current.slice(-4), { type: 'audio/webm' });
            await sendAudioChunk(combinedChunks);
          }
        }
      };

      // Start recording with timeslices of 5 seconds
      ...
      // Auto-stop after 12 seconds if no result found
      setTimeout(() => {
        if (mediaRecorderRef.current?.state === 'recording') {
          stopRecording();
          if (!success) {
            setError('No song detected. Please try again with a clearer audio.');
          }
        }
      }, 12000);
                  ...
                  {isRecording ? (
                    <div className="flex flex-col items-center">
                      <div className="text-white font-bold animate-pulse">
                        {success ? "Song Found!" : isSearching ? "Listening for music..." : "Recording..."}
                      </div>
                      {!success && <div className="text-white">{formatTime(recordingDuration)}</div>}
                    </div>
                  ) : audioURL ? (
                    <div className="flex flex-col items-center">
...
                Processing... */}
      ...
      {/* Detected Song Details - Now as Overlay */}
      {success && result && result.result && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-80">
          <div className="relative w-full h-full max-w-7xl mx-auto p-6 flex items-center justify-center">
